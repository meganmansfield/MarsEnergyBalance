...
%% Use illumination lookup table and obliquity track to construct illumination history.
clear all,close all

%load lookup table
load('illumination_lut_single_precision.mat'); %flat surface at equator or 20 latitude
%load('illumination_lut_sloped.mat'); %sloped surface at 20 latitude
%load('illumination_lut_40sloped.mat') %surface at 40 latitude

for n=[1,2,3,4,5,6,7,8] %for each obliquity track
    % read in data
    [time,semimajor,ecc,inclination,RAascend,no1,argumentperi,obl,Lp,no2]=textread(strcat('halftmorv',num2str(n)),'%f %f %f %f %f %f %f %f %f %f');
    
    %remove first hundred million years to make tracks independent
    semimajor=semimajor(time>10^8);
    ecc=ecc(time>10^8);
    inclination=inclination(time>10^8);
    RAascend=RAascend(time>10^8);
    no1=no1(time>10^8);
    argumentperi=argumentperi(time>10^8);
    obl=obl(time>10^8);
    Lp=Lp(time>10^8);
    no2=no2(time>10^8);
    ctime=time(time>10^8);
    
    %remove part before the break in the data
    dist=zeros(size(ctime,1)-1,1);
    for i=1:(size(ctime,1)-1)
        dist(i)=ctime(i+1)-ctime(i); %put in years to see distance clearly
    end
    
    badpoints=find(dist>100000); %find data points more than 100000 years from the last data point
    startind=max(badpoints)+1; %index to stop at
    semimajor=semimajor(startind:end);
    ecc=ecc(startind:end);
    inclination=inclination(startind:end);
    RAascend=RAascend(startind:end);
    no1=no1(startind:end);
    argumentperi=argumentperi(startind:end);
    obl=obl(startind:end);
    Lp=Lp(startind:end);
    no2=no2(startind:end);
    ctime=ctime(startind:end);
    
    load('bahcall.mat'); % Solar brightening
    smallt=[0:0.000001:8];
    smallf=interp1(bahcall(:,1),bahcall(:,2),smallt)';
    
    % M = mean heating, D = diurnal heating, P = peak heating
    [iMnet,iDnet,iPnet,timetotoday,lumfraction]=insolation(obl,ecc,Lp,ctime,iM,iP,iD,smallf); %flat surface at equator or 20 latitude
    %[iMnet,iDnet,iPnet,timetotoday,lumfraction]=insolation_sloped(obl,ecc,Lp,ctime,iM,iP,iD,smallf); %sloped surface at 20 latitude
    %[iMnet,iDnet,iPnet,timetotoday,lumfraction]=insolation_40sloped(obl,ecc,Lp,ctime,iM,iP,iD,smallf); %surface at 40 latitude
    
    timesunform=4.56-timetotoday;
    
    save(strcat('insolation',num2str(n),'.mat'),'iMnet','iDnet','iPnet','timetotoday','lumfraction','obl'); %flat surface at equator or 20 latitude
    %save(strcat('insolationsloped',num2str(n),'.mat'),'iMnet','iDnet','iPnet','timetotoday','lumfraction','obl'); %sloped surface at 20 latitude
    %save(strcat('insolation40sloped',num2str(n),'.mat'),'iMnet','iDnet','iPnet','timetotoday','lumfraction','obl'); %surface at 40 latitude

%     To create Figure 2
%     windowsize=100000;
%     b=(1./windowsize)*ones(1,windowsize);
%     a=1;
%     meanobl=zeros(size(obl));
%     meanobl(:,1)=filter(b,a,obl(:,1));
%     
%     figure
%     plot(timesunform,obl)
%     hold on
%     plot(timesunform(50000:end-50000),meanobl(100000:end))
%     hold off
%     xlabel('Time [Gyr]')
%     ylabel('Obliquity [degrees]')
%     title(strcat('Obliquity History, Track ',num2str(n)))
end
%% Given an insolation history, calculate the surface energy balance for an ice surface at T=273.15 K.

for n=[1,2,3,4,5,6,7,8]
    load(strcat('insolation',num2str(n),'.mat')); %flat surface at equator or 20 latitude
    load(strcat('insolationsloped',num2str(n),'.mat')); %sloped surface at 20 latitude
    load(strcat('insolation40sloped',num2str(n),'.mat')); %surface at 40 latitude
    ...
    load('rayleigh.mat')
    SWP_in=zeros(size(Pmat,1),size(Pmat,2),size(iPnet,2));
    SWM_in=zeros(size(Pmat,1),size(Pmat,2),size(iPnet,2));
    SWD_in=zeros(size(Pmat,1),size(Pmat,2),size(iPnet,2));
    alpha=0.3; %albedo
    for timestep=1:size(Pmat,1)
        for alphaval=1:size(Pmat,2)
            pdiffm=abs(pout(46,:)-Pmat(timestep,alphaval)); %difference between pressures in matrix and actual pressure at that timestep and alpha value
            [pminm,iminm]=min(pdiffm);
            scatterm=scatter(46,iminm);
            ...
            pdiffp=abs(pout(1,:)-Pmat(timestep,alphaval)); %difference between pressures in matrix and actual pressure at that timestep and alpha value
            [pminp,iminp]=min(pdiffp);
            scatterp=scatter(1,iminp);
            SWP_in(timestep,alphaval,:)=(1-scatterp).*(1-alpha).*iPnet(timestep,:);% Incoming SW from insolation (peak)
            SWM_in(timestep,alphaval,:)=(1-scatterp).*(1-alpha).*iMnet(timestep,:);% Incoming SW from insolation (mean)
            SWD_in(timestep,alphaval,:)=(1-scatterp).*(1-alpha).*iDnet(timestep,:);% Incoming SW from insolation (diurnal)
        end
    end
    ...
    ka=0.112; %Thermal conductivity of CO2 at approx. -17 Celsius in W/m/K (from http://www.engineeringtoolbox.com/carbon-dioxide-d_1000.html)
    ...
    netforceP=zeros(size(SWP_in,1),size(SWP_in,3),size(green_in,2)); %matrix of values for time x latitude x alpha
    netforceM=zeros(size(SWM_in,1),size(SWM_in,3),size(green_in,2));
    netforceD=zeros(size(SWD_in,1),size(SWD_in,3),size(green_in,2));
    for lat=1:size(SWP_in,3) %for each latitude
        for alph=1:size(green_in,2) %for each alpha value
            netforceP(:,lat,alph)=SWP_in(:,alph,lat)-LW_out+green_in(:,alph)-cond_cool(:,alph)-coolflux(:,alph);
            netforceM(:,lat,alph)=SWM_in(:,alph,lat)-LW_out+green_in(:,alph)-cond_cool(:,alph)-coolflux(:,alph);
            netforceD(:,lat,alph)=SWD_in(:,alph,lat)-LW_out+green_in(:,alph)-cond_cool(:,alph)-coolflux(:,alph);
        end
    end

    save('MavenT',num2str(n),'.mat','netforceM','netforceD','netforceP','timetotoday','timesunform','obl'); %flat surface at equator or 20 latitude
    %save('MavenslopedT',num2str(n),'.mat','netforceM','netforceD','netforceP','timetotoday','timesunform','obl'); %sloped surface at 20 latitude
    %save('Maven40slopedT',num2str(n),'.mat','netforceM','netforceD','netforceP','timetotoday','timesunform','obl'); %surface at 40 latitude
...
%     plot(timesunform,netforceD(:,1,6))
%     hold off
%     xlabel('Time [Gyr]')
%     ylabel('Energy Available for Melting [W/m^2]')
%     legend('\alpha=0','\alpha=3.22','\alpha=4.24','\alpha=5.26')
end
...
for n=[1,2]
    load('Maven40slopedT',num2str(n),'.mat');
    ...
    load('MavenslopedT',num2str(n),'.mat')
...
for n=[1,2]
    %load('MavenT',num2str(n),'.mat')   %flat surface at equator or 20 latitude
    load('MavenslopedT',num2str(n),'.mat')  %sloped surface at 20 latitude
    %load('Maven40slopedT',num2str(n),'.mat')   %surface at 40 latitude
    for alph=1:size(netforceD,3)
        netforceD(obl<40,1,alph)=NaN;   %for 0 or 20 latitude
        netforceP(obl<40,1,alph)=NaN;
        netforceM(obl<40,1,alph)=NaN;
        %netforceD(obl>40,1,alph)=NaN;  %for 40 latitude
        %netforceP(obl>40,1,alph)=NaN;
        %netforceM(obl>40,1,alph)=NaN;
        %netforceD(obl<30,1,alph)=NaN;
        %netforceP(obl<30,1,alph)=NaN;
        %netforceM(obl<30,1,alph)=NaN;
    end
...
% 0 Latitude, No Slope
    ...
    netforceM(obl<40,2,alpha+1)=NaN;
    netforceP(obl<40,2,alpha+1)=NaN;
    goodind=netforceD(:,2,alpha+1)>(15-shifts(alpha));%+1)); %start from point >15
    goodind2=netforceD(:,2,alpha+1)>(0-shifts(alpha));%+1)); %end when you drop below 0
...
% 20 Latitude, No Slope
    ...
    netforceM(obl<40,1,alpha+1)=NaN;
    netforceP(obl<40,1,alpha+1)=NaN;
    goodind=netforceD(:,1,alpha+1)>(15-shifts(alpha));%+1)); %start from point >15
    goodind2=netforceD(:,1,alpha+1)>(0-shifts(alpha));%+1)); %end when you drop below 0
    index=1:size(netforceD,1);
    start=index(goodind); %years in which E>15
    stop=index(goodind2); %years in which E>0
    timediffs=diff(timesunform); %actual timesteps from simulation
...
% 40 Latitude, No Slope
    ...
    netforceM(obl<30,2,alpha+1)=NaN;
    netforceP(obl<30,2,alpha+1)=NaN;
    netforceD(obl>40,2,alpha+1)=NaN;
    netforceM(obl>40,2,alpha+1)=NaN;
    netforceP(obl>40,2,alpha+1)=NaN;
    goodind=netforceD(:,2,alpha+1)>(15-shifts(alpha));%+1)); %start from point >15
...
% 40 Latitude, 20 Degree Poleward Slope
    ...
    netforceM(obl<30,1,alpha+1)=NaN;
    netforceP(obl<30,1,alpha+1)=NaN;
    netforceD(obl>40,1,alpha+1)=NaN;
    netforceM(obl>40,1,alpha+1)=NaN;
    netforceP(obl>40,1,alpha+1)=NaN;
    goodind=netforceD(:,1,alpha+1)>(15-shifts(alpha));%+1)); %start from point >15
...
% 40 Latitude, 20 Degree Equatorward Slope
    ...
    netforceM(obl<30,3,alpha+1)=NaN;
    netforceP(obl<30,3,alpha+1)=NaN;
    netforceD(obl>40,3,alpha+1)=NaN;
    netforceM(obl>40,3,alpha+1)=NaN;
    netforceP(obl>40,3,alpha+1)=NaN;
    goodind=netforceD(:,3,alpha+1)>(15-shifts(alpha));%+1)); %start from point >15
    goodind2=netforceD(:,3,alpha+1)>(0-shifts(alpha));%+1)); %end when you drop below 0
    index=1:size(netforceD,1);
    start=index(goodind); %years in which E>15
    stop=index(goodind2); %years in which E>0
    timediffs=diff(timesunform); %actual timesteps from simulation
